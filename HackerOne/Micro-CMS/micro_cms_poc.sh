...
base = "https://375d9d90f37dd7436583b6e75dbe0550.ctf.hacker101.com"
title = "probe-" + uuid.uuid4().hex[:8]
...
if not location:
    raise SystemExit("No redirect returned after page creation.")

page = requests.get(base + location, timeout=15)
match = re.search(r"\^FLAG\^[^\s<]+\$FLAG\$", page.text)

if not match:
    raise SystemExit("Flag not found in rendered page output.")
